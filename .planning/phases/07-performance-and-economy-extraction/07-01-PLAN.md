...
must_haves:
  truths:
    ...
    - "Phase 6 orchestrator persists opening_kills, opening_deaths, multi_kills, clutch_wins, traded_deaths, round_swing to the new columns"
...
5. Add `upsert_perf_economy_complete(self, perf_stats: list[dict], economy_data: list[dict], kill_matrix_data: list[dict]) -> None` method:
...
6. Update the dict construction in the parse+persist loop (around line 110-133) to include the 7 new columns. Add these keys to the `stats_data.append({...})` dict:
   ```python
...
Also verify the new methods exist: `python -c "from scraper.repository import MatchRepository, UPSERT_KILL_MATRIX, GET_PENDING_PERF_ECONOMY; print('Imports OK')"`
  ...
  <done>UPSERT_PLAYER_STATS includes all 7 new columns. UPSERT_KILL_MATRIX and GET_PENDING_PERF_ECONOMY constants exist. Repository has get_pending_perf_economy() and upsert_perf_economy_complete() methods. Phase 6 orchestrator dict includes opening_kills through round_swing. All existing tests pass.</done>
</task>
...
- Repository has get_pending_perf_economy() and upsert_perf_economy_complete() methods
